...
import com.heheys.ec.view.RefreshableView.PullToRefreshListener;
...
public class ShoppingCartFragment extends BaseFragment implements
	...
	private static final String TAG = ShoppingCartFragment.class.getName();
...
//	public ShoppingCartFragment(MainActivity mainActivity,ImageView ivShoppingCart) {
//		super();
...
//	}
	...
	public void onStart() {
		...
		isLogin = IsLogin.isLogin(mainActivity);
	}
...
/**
	...
	private void initView(View parentView) {
		linear_need_buy = (LinearLayout) parentView
				.findViewById(R.id.base_fragment_shopcart_nodata);
		no_shopcart_buy = (TextView) parentView//去逛逛视图
				.findViewById(R.id.base_activity_no_shopcart_buy);//去逛逛按钮
		tv_no_login = (Button) parentView
				.findViewById(R.id.base_activity_no_shopcart_login);//登录按钮
		linear_no_login = (LinearLayout) parentView
				.findViewById(R.id.base_acticity_no_login_parent);//需要登录视图
		...
		tvTotalNumPriceTextView = (TextView) parentView
				.findViewById(R.id.fragment_shopping_cart_bottom_num);
		...
		tv_allcheck = (TextView) parentView
				.findViewById(R.id.tv_allcheck);
...
//		scrollView = (RefreshScrollView) parentView
...
//		scrollView.setonRefreshListener(this);
		...
		if( mainActivity !=null && !StringUtil.isEmpty(mainActivity.isShowBack)){
		if (ConstantsUtil.SHOPPING_CART_SHOW_BACK
				.equals(StringUtil.isEmpty(mainActivity.isShowBack)?"shopping_cart_show_back":mainActivity.isShowBack) && mainActivity!=null) {
			*/
/**
			 * 显示返回键
			 *//*
			...
			mainActivity.llTabParent.setVisibility(View.GONE);
				...
				.equals(StringUtil.isEmpty(mainActivity.isShowBack)?"shopping_cart_not_show_back":mainActivity.isShowBack) && mainActivity!=null) {
			*/
/**
			 * 不显示返回键
			 *//*
...
/*
	...
	private void DeleteGoods(){
		...
		if (deleteProductList != null&& deleteProductList.size() > 0) {
			...
			mDialogCoustom.AlertShoppingError(2, baseActivity, "确认要删除这"+deleteProductList.size()+"种商品吗?", new UpdateOrNot() {
				@Override
...
/**
				...
				if (mAdapter.dataList != null && mAdapter.dataList.size() > 0) {
					...
					btSettlement.setBackgroundResource(R.drawable.pay_bt);
					tvRight.setText("完成");
...
/**
	...
	public static class DeleteProductMessageHandler extends
			WeakHandler<ShoppingCartFragment> {

		public DeleteProductMessageHandler(ShoppingCartFragment reference) {
			super(reference);
			// TODO Auto-generated constructor stub
		}
...
//					StartActivityUtil.startActivity(mainActivity, LoginActivity.class);
//					LoginSampleHelper.getInstance().loginOut_Sample(getReference().baseActivity);
					getReference().mainActivity.finish();
				}else{
...
//				MainActivity.btProductNum.setVisibility(View.GONE);
//			}else{
//				MainActivity.btProductNum.setText(getTotalNum() + "");
//				
//			}
		} else {
		}
...
/**
	...
	//设置购物车数据
	void setShoppingcar(){
		if (shoppingCartData != null && shoppingCartData.getResult() instanceof NewShoppingCartInfor) {
...
/**
	...
	public static class ShoppingCartMessageHandler extends
		...
		public ShoppingCartMessageHandler(ShoppingCartFragment reference) {
			super(reference);
			// TODO Auto-generated constructor stub
		}
...
/**
				...
				if(num[1] > 0){
					...
					getReference().mainActivity.btProductNum.setVisibility(View.VISIBLE);
					getReference().btSettlement.setEnabled(true);
					getReference().btSettlement.setBackgroundResource(R.drawable.pay_bt);
				}else{
					...
					getReference().mainActivity.btProductNum.setVisibility(View.INVISIBLE);
				}
				if(num[0]>0){
				 getReference().mainActivity.btProductNum.setText(num[0] + "");
				 getReference().mainActivity.btProductNum.setVisibility(View.VISIBLE);
...
/**
				...
				if (msg.obj != null && msg.obj instanceof NewShoppingCartInfor) {
					...
					ShoppingCartFragment.productListData = getReference().shoppingCartInfor.getList();
					if (ShoppingCartFragment.productListData != null&& ShoppingCartFragment.productListData.size() > 0) {
...
/**
						...
						getReference().linear_no_login.setVisibility(View.GONE);
						getReference().mainActivity.btProductNum.setVisibility(View.INVISIBLE);
						if(!IsLogin.isLogin(getReference().baseActivity)){
							Utils.StartLogin(getReference().mainActivity, true);
						}
					}
...
/*
						...
						if(selectProductNotInEditTemp!=null && selectProductNotInEditTemp.size()>0){
						  mDialogCoustom.AlertShoppingError(1,baseActivity, StringUtil.isEmpty(ErrorBeanorder.getTips())?"":ErrorBeanorder.getTips(),new UpdateOrNot() {
							@Override
							public void setResult(int modle) {
								if(2 == modle){
...
//					ToastUtil.showToast(baseActivity, StringUtil.isEmpty(ErrorBeanorder.getInfo())?"获取失败":ErrorBeanorder.getInfo());
					mDialogCoustom.AlertShoppingError(3,baseActivity, StringUtil.isEmpty(ErrorBeanorder.getTips())?"":ErrorBeanorder.getTips(),new UpdateOrNot() {
						@Override
						public void setResult(int modle) {
							mDialogCoustom.Demiss();
						}
...
/*	class MyShoppingCallBack extends BaseJsonHttpResponseHandler<NewOrderBaseBean> {
	...
	public class MyOrderHandler extends WeakHandler<ShoppingCartFragment> {

		@SuppressLint("HandlerLeak")
		public MyOrderHandler(ShoppingCartFragment reference) {
			super(reference);
		}
...
//					getActivity(), this, shoppingCartMessageHandler, lvListView,ivShoppingCart);
...
////					mainActivity.btProductNum.setText(getTotalNum() + "");
//					MainActivity.btProductNum.setText(NewShoppingCartFragmentAdapter.totalNum + "");
//					
//				}
...
/**
	...
	public void cancelAllProductNotInEdit() {
		if (selectProductNotInEdit != null) {
			...
			btSettlement.setBackgroundColor(mainActivity.getResources().getColor(R.color.color_888888));
		}
	}
