...
//	private boolean mLock;
	 ...
	 */
	private OnFooterLoadMoreListener mOnFooterRefreshListener;
	/**
	 * footer refresh listener
	 */
	...
	public boolean onInterceptTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			int  destance = ViewConfiguration.get(mContext).getScaledTouchSlop();
			if(Math.abs(deltaY) < 8){
				//左右位移是8个像素 减少误差率 不拦截点击
				return false;
			}
...
//							+ mFooterViewHeight) {
	...
	private boolean isRefreshViewScroll(int deltaY) {
		...
		if (mAdapterView != null) {
			...
			if (deltaY > 0) {
				...
				if (mAdapterView.getFirstVisiblePosition() == 0
						&& Math.abs(top - padding) <= 5) {// 这里之前用3可以判断,但现在不行,还没找到原因
					mPullState = PULL_DOWN_STATE;
					return true;
				}
				...
				if (lastChild.getBottom()-20 <= getHeight()
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >=  mFooterViewHeight
//				if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight)
	...
	public void setOnFooterRefreshListener(
			OnFooterLoadMoreListener footerRefreshListener) {
