...
//			}
			 ...
			 * 
			 * **/
			ApiHttpCilent.getInstance(getApplicationContext()).RegisterApp(context, tellength, pwd_one, codelength,invitecode, new MyCallBack());
			break;
		...
		case R.id.base_activity_title_right_righttv:
			   StartActivityUtil.startActivity((Activity) context,new Intent(context,LoginActivity.class));
				break;
		default:
			break;
		}
...
//		}
		...
		isCode = 1;//1 提示获取验证码成功
 		ApiHttpCilent.getInstance(getApplicationContext()).ObtinCode(context, tellength, new MyCallBack(),ConstantsUtil.CODE_FLAG_1);//获取验证码
	}
...
//			loginService(userid,loginObj.getImpassword());
		 ...
		 loginObj = loginBean.getResult();
		    ...
		    isCode = 4;
			ApiHttpCilent.getInstance(getApplicationContext()).BindCid(RegisterActivity.this, loginObj.getId(), cid, new MyCallBack());
