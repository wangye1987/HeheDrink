...
//		tvRight.setTextSize(ViewUtil.px2sp(baseActivity, 28));
		...
		//获取版本信息
		ApiHttpCilent.getInstance(getApplicationContext()).initVersiondata(context, "android",new VersionCallBack());
	}
...
//								ToastUtil.showToast(baseActivity, "更新aspp");
	...
	public void onClick(View v) {
		...
		case R.id.commit_verify:
			...
			flag = 0;
			ApiHttpCilent.getInstance(getApplicationContext()).login(context,
...
//				 loginObj=(ResultBean) msg.obj;
//				//跳转到首页
//				String account = Md5Util.MD5(loginObj.getMobile());
...
//			loginService(userid,loginObj.getImpassword());
			...
			SharedPreferencesUtil.writeSharedPreferencesString(context, "loginaccount", "account", Md5Util.MD5(loginObj.getMobile()));
			Intent intent = new Intent();
			intent.setAction(ConstantsUtil.LOGIIN_ACTION);
			context.sendBroadcast(intent);
				if( MyApplication.getInstance().loginCallBack != null){
					MyApplication.getInstance().loginCallBack.loginSuccess();
					getReference().finish();
				}else{
...
//			}
			...
			flag = 1;
			ApiHttpCilent.getInstance(getApplicationContext()).BindCid(LoginActivity.this, loginObj.getId(), cid, new MyCallBack());
...
//			map.put("retentedPeriod",loginObj.getRegdays());
			MobclickAgent.onEvent(baseContext, "C_LGN_1", map);
