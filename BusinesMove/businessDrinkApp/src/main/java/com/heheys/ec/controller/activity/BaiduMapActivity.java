...
//		BitmapDescriptor bitmap = Drable(i);
		OverlayOptions textOption = new TextOptions()
				.fontSize(24).fontColor(0xFFFF3838).text(StringUtil.isEmpty(addName)?"***":addName)
				.position(point);
		// 在地图上添加Marker，并显示
		mBaiduMap.addOverlay(textOption);
		
		MarkerOptions ooD = new MarkerOptions().position(point).icon(bitmapDescriptor).zIndex(0);
		// 生长动画
