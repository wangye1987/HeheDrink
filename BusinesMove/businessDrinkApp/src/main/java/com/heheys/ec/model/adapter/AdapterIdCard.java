...
/** 
 * @author 作者 E-mail: 
 * @version 创建时间：2015-11-11 下午4:33:15 
 * 类说明 
 * @param
 */
public class AdapterIdCard extends BaseAdapter{
...
//	private ArrayList<Drawable> mDrawableList = new ArrayList<Drawable>();
	...
	ClickCallBacks clickBack;
	TextView tv_edit_idcard;
	...
	public AdapterIdCard(Context context,boolean isRegister, List<String> nameList,String baseurl,TextView tv_edit_idcard,ImageCallBack ivcallback
			,int role,int viplevel, String statusAuth) {
		...
		this.tv_edit_idcard = tv_edit_idcard;
		pics.clear();
	...
	public void isEdit(boolean isEdit,ClickCallBacks clickCallBack){
		this.isEdit = isEdit;
		this.clickBack  = clickCallBack;
		if(!isEdit){
			if(!mNameList.contains("") && mNameList.size()<2)
			mNameList.add("");
		}
	}
	public int getCount() {
	...
	public void setData(List<String> nameList,boolean isReplace){
		...
		if(mNameList.size()<2 && mNameList.size()!=2){
			if(!isReplace)
			mNameList.add("");
			}
	}
				...
				viewTag.mLinear_delete.setOnClickListener(new OnClickListener() {
					public void onClick(View v) {
						...
						if(mNameList.size()==1 && mNameList.get(0).equals("")){
							auth.IsCanClick(mContext,tv_edit_idcard, mNameList);
...
//			if(role!=1)
//			viewTag.icon.setOnClickListener(auth.new MyClassClick(position+6,viewTag.icon,mContext, ivcallbacks));
		}
//		if(!isRegister){
			viewTag.icon.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					mContext.startActivity(intent);
						...
						viewTag.mLinear_delete.setVisibility(View.GONE);
//						if(role!=1)
							viewTag.icon.setOnClickListener(auth.new MyClassClick(index+6,viewTag.icon,mContext, ivcallbacks));
							MobclickAgent.onEvent(mContext, "C_AUTH_2");
					}
				}
			});
//		}
