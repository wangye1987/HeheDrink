	...
	if(rechargeoid != null && 	rechargeoid.getResult() != null && 	!StringUtil.isEmpty(rechargeoid.getResult().getOid())){
		...
		if ( ConstantsUtil.JD_PAY.equals(payType)) {
			...
			if(api.isWXAppInstalled() && api.isWXAppSupportAPI()){
				ApiHttpCilent.getInstance(getApplicationContext()).WXPaySign(mContext, oid,"","wechat", new WXPayCallBack());
			}else{
...
//		ForegroundColorSpan colorSpan = new ForegroundColorSpan(getResources().getColor(R.color.color_af2942));
	...
	class MyClickText extends ClickableSpan{
        ...
        public void updateDrawState(TextPaint ds) {
            ...
            //设置文本的颜色
            ds.setColor(getResources().getColor(R.color.color_FFd838));
	...
	protected void getNetData() {
		// TODO Auto-generated method stub
		//我的喝喝币 
		ApiHttpCilent.getInstance(getApplicationContext()).UserHeheMoney(baseActivity, new MyHeheMoneyCallBack());
		//可以充值的喝喝币 
		ApiHttpCilent.getInstance(getApplicationContext()).erchangeHeheMoney(baseActivity, new HeherechargeCallBack());
	}
	...
	public void onClick(View v) {
		...
		if(needPay == 0){
			...
			ApiHttpCilent.getInstance(getApplicationContext()).crchangeCard(mContext,coinid,num, payType, new RechargeCallBack());
			break;
		case R.id.help:
