...
//			url = "";
	...
	public void postHeart(Context context, RequestParams param, String method,
					 ResponseHandlerInterface responseHandler) {
		String url = RequestConfiguration.BASE_URL_TEST_LACALHOST_HEART + method;
		PackageManager pm = context.getPackageManager();
		try {
			PackageInfo info = pm.getPackageInfo(context.getPackageName(), 0);
			param.put("version", info.versionCode);//versionName
		} catch (PackageManager.NameNotFoundException e) {
			e.printStackTrace();
		}
//		param.put("version", "17");//versionCode
		param.put("os", "android");
		param.put("factory", android.os.Build.MODEL);
		param.put("os_version", android.os.Build.VERSION.RELEASE);
//		Log.i("Tag", "请求地址" + url  + param);
		asyncHttpClient.post(context, url, param, responseHandler);
	}
