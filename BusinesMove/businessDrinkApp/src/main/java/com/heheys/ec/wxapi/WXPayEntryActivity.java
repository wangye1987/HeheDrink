...
//				"status":"" 0、没有订单支付记录1、支付成功，可退款，2、支付失败，3、等待支付 4、支付超时关闭，5、交易结束，不可退款 
				...
				if(listbean_new_list!=null && listbean_new_list.size()>0){
					...
					for(int i=0;i<orderSize;i++){
						...
						tv_order_num.setText("订单号: "+listbean_new_list.get(i).getOrderNum());
...
//		Log.d("TAG", "onResp------"+resp.toString()+"----"+resp.getType()+"---"+resp.transaction+"--"+resp.openId);
		...
		boolean isOrder = MyApplication.getInstance().isOrder();
		//订单号
		...
		if(!StringUtil.isEmpty(oid)){
			if(isOrder){
				//正常订单结果请求接口
				ApiHttpCilent.getInstance(getApplicationContext()).SignPayResult(this, oid, new PayCallBack());
			}else{
				//喝喝币充值结果请求接口
